...
token_scores <- function(args) {

  # Initialise attack keywords
  anomalytic::attack_keywords()
  attackNames <- attackWords %>% names

  # Tokenize stuff
  args %<>% tolower
  tokens <- args %>% anomalytic::tokenization()

  # Create character distribution
  chardist <- args %>% anomalytic::char_dist()

  # Split by spaces / collapse string / create a matrix of column 1
  scores <- tokens %>%
    sapply(anomalytic::max_from_list) %>%
    matrix(ncol = 1)

  matching <- anomalytic::parse_words(
    tokens = tokens,
    args = args
  )

  # Check for doubled words like `make set` etc..
  matching %<>% anomalytic::double_words()
  ...
  cbind(scores, escores, chardist, counts) %>% return()
}
